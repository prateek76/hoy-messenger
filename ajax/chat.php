...
if(isset($_POST['method']) === true && empty($_POST['method']) === false) {
            ...
            echo "there are no users";
                ...
                ?>
                            ...
                            if(isset($_SESSION['u_id']))
                            {
                                ...
                                $fp = fopen('../users/XvrkBpAEWtgfpz__'.$_SESSION['username'].'.json', 'w');
                                fwrite($fp, json_encode($friends));
                                fclose($fp);
                                ?>
                                <?php
                            }  ?>
    ...
    if($method === 'fetch' && isset($_SESSION['u_id'])) {
        ...
        if(empty($messages) === true) {
            ...
            foreach($messages as $message) {
                ...
                if ($message['user_id'] == $_SESSION['u_id'] && $message['send_id'] == $send_id) {
                    ...
                    $row[] = $message;
                    //$message_todel[] = $message['message_id'];
                }
            }
            echo json_encode( $row );
            if(!file_exists('../messages/'))
                mkdir('../messagesOneToOne', 0700);
            $fp = fopen('../messagesOneToOne/XvrkBpAEgyuFmAvvYGGXnCAUljl7APtmfHWtgfpz__'.$send_id.'.json', 'w');
            fwrite($fp, json_encode($row));
            fclose($fp);
            /*for ($i=0; $i < sizeof($message_todel); $i++) {
                $chat->delete($message_todel[$i]);
            }*/
        }

    } else if($method === 'fetchGRP' && isset($_SESSION['u_id'])) {
        //fetch messages and output
        $send_id = $_POST['send_id'];
        $messages = $chat->fetchMessages(/*$send_id,$_SESSION['u_id']*/);
        //print_r($messages);
        ...
        $flag_sub = 0;
        $subscripTion = new Chat();
        $currentSUB = $subscripTion->current_subscription($_SESSION['u_id']);
        $currentSUB = explode('__', $currentSUB[0]['group_subscription']);

        for ($i=0; $i < sizeof($currentSUB); $i++) {
            if($currentSUB[$i] == $send_id) {
                $flag_sub = 1;
                break;
            }
        }
        ...
        if(empty($messages) === true) {
            ...
            foreach($messages as $message) {
                ...
                if ($message['user_id'] == $_SESSION['u_id'] && $message['send_id'] == $send_id) {
                    ...
                    //$data .= '{"message":'.print $message['message'].',"sendto":'.print $message['user_id'].'},';
                    $row[] = $message;
                } else if ($message['send_id'] == $send_id && $message['user_id'] != $_SESSION['u_id']) {
                    $row[] = $message;
                }
            }
            echo json_encode( $row );
            $fp = fopen('../chats/XvrkBpAEgyuFmAvvYGGXnCAUljl7APtmfHWtgfpz__'.$send_id.'.json', 'w');
            fwrite($fp, json_encode($row));
            fclose($fp);
            return;
        }
        ...
        if($send_id === '235') {
            if(empty($message) === false) {
                $chat->throwMessages($_SESSION['u_id'], $send_id, $message);
                $reply = $chat->reply($message);
                if($reply != "")
                    $chat->throwMessages(235, $_SESSION['u_id'], $reply);
            }
        } else {
            if(empty($message) === false) {
                //throw it
    ...
    } else if ($method === 'throwGRP' && isset($_POST['message']) === true) {
        //throw messages into database
        //make sure user has subscription
        /**//**/
        /**/
        /**/
        /**//**/
        ...
        //MAKE HERE
        $flag_sub = 0;
        $currentSUB = $chat->current_subscription($_SESSION['u_id']);
        //echo($currentSUB[0]['group_subscription']);
        $currentSUB = explode('__', $currentSUB[0]['group_subscription']);

        $message = trim($_POST['message']);
        $send_id = $_POST['send_id'];
        $senders_nameGRP = $_SESSION['username'];
        ...
        if(empty($message) === false && $flag_sub == 1) {
            //throw it
            $chat->throwMessagesGRP($_SESSION['u_id'], $send_id, $senders_nameGRP, $message);//error here
            $flag_sub = 0;
        } else {
            echo 'copypastekartahaimj';
        }
        ...
        //updated both users table and groups table
        if(!empty($group_name) && $group_name != '0') { //empty() returs true if 0 is used
         ...
         * Test data
         */
        //$group_id length should be 20
        //$test_group = "Bwost8dlYgjLifNG(ygD";
        $group_id = $_POST['group_id'];//send_id se lo
        ...
        if ($ifUserIsAdmin == 69) {
            ...
            /***
                NEW ADDED FOR SECURITY
            /**/
            for ($i=0; $i < sizeof($group_users); $i++) {
                if($user_id == $group_users[$i]) {
                    echo "Alredy a member";
                    return;
                }
            }
            ...
            $chat->add_member_to_the_group($noofusers,$group_id,$group_users,$user_id,$subscription_list);

            echo "added new usere";
            return 0;//no use
        } else {
            return 0;//no use
        }
        ...
        $fp = fopen('../groups/XvrkBpAEWtgfpz__'.$_SESSION['username'].'.json', 'w');
        fwrite($fp, json_encode($group_all_data));
        fclose($fp);
    } else if ($method === 'showAddOption') {
        //show all users
        $group_id = $_POST['group_id'];
        //$group_id = "Bwost8dlYgjLifNG(ygD";
        $users = $chat->fetchUsers();
                            ...
                            if(isset($_SESSION['u_id']))
                            {
                                ...
                                foreach ($users as $user) {
                                    if($user['user_id'] != $_SESSION['u_id']) {
                                        ...
                                        //
                                        $rat = new Chat();
                                        $availableORnot = $rat->current_subscription($user['user_id'])[0]['group_subscription'];
                                        //print_r( $availableORnot[0]['group_subscription'] );
                                        //echo "\n";
                                        $availableORnot = explode("__", $availableORnot);
                                        for ($i=0; $i < sizeof($availableORnot); $i++) {
                                            if($group_id == $availableORnot[$i]) {
                                                $availableBool = "NO";
                                                break;
                                            }
                                        }
                                        $availableToadd[] = array($user__unique__id => $user['user_id'],$username => $user['username'],"addornot" => $availableBool);
                                ?>
                                <?php }}
                                echo json_encode($availableToadd);
                                ?>
                                <?php
                            }  ?>
                <?php
